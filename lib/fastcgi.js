...
function Parser() {
	...
	_parser._record = record;

	var _header = new Buffer(constants.general.FCGI_HEADER_LEN);
	var _body = _parser._body = new Buffer(constants.general.FCGI_MAX_BODY);
...
*/
// TODO: I think this should be using an Object hash instead of an array of arrays,
// seeing as parameters cannot (or should not) be duplicated. - Sam
var getParamLength = exports.getParamLength = function(params) {
	var size = 0;
	params.forEach(function(param) {
		size += (param[0].length + param[1].length);
		if(param[0].length > 127) {
...
// Higher level API to read/write FCGI records.
exports.records = {};

var RecordBEGIN = exports.records.BEGIN = function(role, flags) {
	this.type = constants.record.FCGI_BEGIN;
	this.role = role || constants.role.FCGI_RESPONDER;
	this.flags = flags || 0;
};

RecordBEGIN.prototype.getSize = function() {
	return 8;
};

RecordBEGIN.prototype.write = function(writer) {
	writer.writeBegin({
		role: this.role,
		flags: this.flags
	});
...
RecordBEGIN.prototype.parse = function(body) {
	this.role = body.role;
	this.flags = body.flags;
};

var RecordEND = exports.records.END = function(appStatus, protocolStatus) {
	this.type = constants.record.FCGI_END;
	this.appStatus = appStatus || 0;
	this.protocolStatus = protocolStatus || 0;
};


RecordEND.prototype.getSize = function() {
	return 8;
};

RecordEND.prototype.write = function(writer) {
	writer.writeEnd({
...
var RecordPARAMS = exports.records.PARAMS = function(params) {
	this.type = constants.record.FCGI_PARAMS;
	...
	if(params) {
		if(Array.isArray(params))
			...
			Object.keys(params).forEach(function(paramName) {
				this.params.push([paramName, params[paramName]]);
			}.bind(this));
		}
	};
};

RecordPARAMS.prototype.getSize = function() {
	return getParamLength (this.params);
};

RecordPARAMS.prototype.write = function(writer) {
	writer.writeParams(this.params);
};

RecordPARAMS.prototype.parse = function(record) {
	Object.keys(record.params).forEach(function(paramName) {
		this.params.push([paramName, record.params[paramName]]);
	}.bind(this));
};

// Returns parameters as an object map rather than array.
RecordPARAMS.prototype.getObject = function() {
	var paramsObj = {};
...
Writer.prototype.writeRecord = function(record, requestId) {	
	var header = {
		version: constants.version,
		type: record.type,
		recordId: requestId,
		contentLength: record.getSize(),
		paddingLength: 0 // TODO: should we align stuff on 8byte boundaries? Is that even necessary these days?
	};
...
// Lookup table for record objects.
var recordsByType = {};
recordsByType[constants.record.FCGI_BEGIN] = RecordBEGIN;
recordsByType[constants.record.FCGI_END] = RecordEND;
recordsByType[constants.record.FCGI_PARAMS] = RecordPARAMS;

// Different version of the Parser that receives record objects instead of raw data.
var Client = exports.Client = function() {
	Parser.call(this);
	
	var super_parseBody = this._parseBody;
	this._parseBody = function() {
		// Use original body parser, but don't let it emit.
		var onRecord = this.onRecord;
		this.onRecord = null;
		super_parseBody.call(this);
		
		// Restore handler.
		this.onRecord = onRecord;

		// Create the proper record object and emit it.
		var newRecord = new recordsByType[this._record.header.type];
		newRecord.parse(this._record.body);

		// Note that onRecord now also spits out request ID as a param.
		this.onRecord(newRecord, this._record.header.recordId);
	};
};
require("util").inherits(Client, Parser);