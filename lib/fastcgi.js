				...
				case HEADER:
					if(_loc == FCGI_HEADER_LEN - 1) {
						...
						if(_record.header.contentLength > 0) {
							if(_parser.onHeader) _parser.onHeader(header);
							_parser.state = BODY;
						}
						else {
							if(_parser.onRecord) _parser.onRecord(_record);
						}
						_loc=0;
					}
					else {
					...
					if(_loc == _record.header.contentLength - 1) {
						...
						switch(_record.header.type) {
							...
							case FCGI_GET_VALUES_RESULT:
								...
								_record.body.params = {};
								while(j < _record.header.contentLength) {
									nlen = _body[j];
									if(nlen >> 7 == 1) {
									...
									if((j + nlen + vlen) <= _body.length) {
										...
										if(_parser.onError) _parser.onError(new Error(JSON.stringify(BUFFER_OVERRUN)));
										j = _record.header.contentLength;
									}
								}
								break;
							case FCGI_STDIN:
							...
							case FCGI_DATA:
								switch(_parser.encoding) {
									...
									case "ascii":
										...
										break;
									default:
										if(_parser.onBody) _parser.onBody(_body, 0, _record.header.contentLength);
										break;
								}
								break;
